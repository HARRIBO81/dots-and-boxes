...
public class Board extends JPanel
{
    ...
    {
        super( true );
        JFrame f = new JFrame( "Dots And Boxes by Ryan Javid" );
        ...
        for (int i = 0; i < this.grid.length; i++)
        {
            for (int j = 0; j < this.grid[i].length; j++)
            {
                ...
                if (curColour != null) // Draw pegs if they exist
                {
                    ...
                    g.fillOval(curX+deltaX/4,curY+deltaY/4,15,15);
                }
            }
        }
    ...
    private void drawLine(Graphics g)
    {
        for (int i =0; i < numLines; i++ )
        {
            ...
            g.drawLine( (int)Math.round((X_OFFSET+X_DIM/2.8+line[0][i]*X_DIM)*scale),
                    (int)Math.round((Y_OFFSET+Y_DIM/2.8+line[1][i]*Y_DIM)*scale),
                    (int)Math.round((X_OFFSET+X_DIM/2.8+line[2][i]*X_DIM)*scale),
                    (int)Math.round((Y_OFFSET+Y_DIM/2.8+line[3][i]*Y_DIM)*scale) );
//            g.drawLine(60,52,105,52);
    ...
    public Coordinate getClick()
    {
        ...
        synchronized(this) {
            ...
            int col = lastClick.getCol();
            int row = lastClick.getRow();
